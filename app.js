...
function openUmbrellaCatalog(){
  ...
  const choices=payments.map((p,i)=>'<button class="payment-method '+p[0]+' '+(!i?'selected':'')+'" data-payment="'+p[0]+'"><i>'+p[3]+'</i><span><b>'+p[1]+'</b><small>'+p[2]+'</small></span><strong aria-hidden="true">'+(!i?'✓':'')+'</strong></button>').join('');
  modal('เลือกวิธีชำระเงิน','<div class="umbrella-catalog payment-sheet"><div class="catalog-head"><span>จุดรับ · '+selectedStation.name+' · ตู้ '+selectedStation.locker+'</span><b>เช่าร่ม RǪM Standard</b><small>ฟรี 30 นาทีแรก · เลือกช่องทางที่สะดวก</small></div><div class="payment-security"><span>☂ รับร่มภายใน 5 นาทีหลังชำระ</span><b>🔒 Demo ที่ปลอดภัย</b></div><div class="payment-methods" role="radiogroup" aria-label="ช่องทางชำระเงิน">'+choices+'</div><button id="confirmUmbrella" class="catalog-confirm">ยืนยันและจองร่ม <span>→</span></button><p class="catalog-note">ข้อมูลการชำระเงินเป็น Demo · ไม่มีการเรียกเก็บเงินจริง</p></div>');
